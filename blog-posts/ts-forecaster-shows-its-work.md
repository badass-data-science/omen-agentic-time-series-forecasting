...
*In which a hypothesis test confirms a model's apparent win is a real one instead of holdout luck, a grid search finds a numerically perfect candidate with structurally broken residuals, and a bug gets caught in the math before it ever reached production.*
...
Want to actually work through this layer's tools yourself, with real worked examples? Chapters 8-13 of *Agentic Time Series Forecasting for Supervillains* (`../book/`) cover `ts-forecaster` in depth — baselines through Diebold-Mariano testing and rolling-origin backtesting.
