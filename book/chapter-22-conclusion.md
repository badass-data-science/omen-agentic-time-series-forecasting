...
- **No standing schedule for `ts-monitor` checks.** Every drift check, every degradation comparison, every `recommend_retraining` verdict in this book happened because a human or an agent decided, in the moment, to ask. Nothing in the toolkit runs `ts-monitor` on a cadence of its own — a deployed forecast can quietly go stale for as long as nobody thinks to check it, which is a strange gap in a project this insistent, everywhere else, on verifying rather than assuming.
- **No exogenous-regressor support anywhere in the toolkit.** Every model this book has fit — naive, ETS, SARIMA, gradient-boosted trees — forecasts a series from nothing but its own past values and the calendar. None of them can take "and there was also a price change" or "a promotion ran that week" as an input. For a series whose future genuinely depends on something other than its own history, that's a real ceiling on how far any of Part III's models can go.
