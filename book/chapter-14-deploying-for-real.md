...
The fix is what Chapter 3 already told you was missing: interpolate the 5 gaps before doing anything else with this series.
...
**What It Means:** The caveat names its own interval as **quantile-regression-based** — precise about what that means: it's a structurally different construction from ETS's simulated paths or SARIMA's formula-driven one earlier in this chapter. A quantile regression model is trained to directly predict a specific percentile of the outcome — the 5th and 95th, say — rather than predicting a single mean and assuming a spread around it. That's a real, structurally different way to build an interval, and it comes with its own blind spot, named plainly in the caveat: it isn't derived from the same step-by-step recursive process that's actually generating the point forecasts, so it has no particular reason to widen just because those recursive predictions are compounding on themselves.

The interval barely moved — `63.47` wide at day 5, `59.71` wide at day 90, eighty-five days later. That's the caveat above made concrete: this interval does not grow with the compounding risk of forecasting recursively off the model's own earlier guesses. The point forecast itself deserves a second look too: the 90-day trajectory doesn't visibly blow up or drift somewhere absurd — inspecting the final five days (`267.6, 242.0, 227.9, 210.3, 208.9`) shows almost the same weekly oscillation shape as the first five days (`233.7, 267.5, 261.6, 242.2, 217.9`). That's not reassuring once you understand *why* it happens: with only `lag_1`, `lag_7`, and `lag_14` as features and (per Chapter 11's real finding on a different series) `lag_7` dominating everything else, this model doesn't really extend a trend or reason about the future — it recursively re-plays the most recent strong repeating pattern it has memory of, forever. That's a perfectly reasonable forecast if the future keeps looking like the recent past. It is *not* a model that will visibly warn you the moment that assumption breaks — it'll keep confidently repeating the old pattern instead, with an interval that, as just shown, doesn't widen to reflect the extra distance from anything real.
