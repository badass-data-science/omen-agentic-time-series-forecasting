...
**What Comes Back** (real result, searching `p` and `q` from 0–2 with `d=1` fixed and seasonal terms disabled, as reasoned above):
...
**What It Means:** A search over just 9 combinations found `(1,1,2)`, cutting MAPE from the guessed model's `15.08%` down to `7.23%` — a real, substantial improvement, and every single top candidate here happens to pass its own residual check. This chapter isn't going to pretend that's guaranteed: a best-AICc candidate can just as easily come back with broken residuals — ranked first by the numbers, structurally deficient underneath them — and nothing about this search would have warned you in advance which outcome you'd get. That's why the check has to run *every* time regardless of how clean the ranking looks, not just when something already seems suspicious. A residual check you only bother running when you already expect trouble isn't really a check.

Look past just the top two candidates, and Chapter 9's AICc-versus-MAPE trap shows up again in this exact result, quieter the second time: third-ranked `(0,1,2)` has the better AICc (`560.33` versus `573.18`), but the worse MAPE (`16.781%` versus fourth-ranked `(2,1,1)`'s `14.769%`). AICc doesn't even reliably order these five candidates by the accuracy this chapter actually cares about — it's not just a top-two-near-tie problem.

Look instead at what this search's own `interpretation` flagged on its own: the top two candidates, `(1,1,2)` at AICc `546.52` and `(2,1,2)` at AICc `547.12`, are separated by less than a single point — a margin the tool itself calls out as close enough to warrant a proper significance test rather than trusting the ranking blind. That's Chapter 12's job, not this chapter's, and this search result is the kind of situation it exists for.
