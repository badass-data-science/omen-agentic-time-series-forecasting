...
Omen is organized as five layers, each one answering a different question in the life of a forecast. They're meant to be used roughly in this order, and this book teaches them in roughly this order too, because it turns out to be a genuinely sound way to learn forecasting generally, not just this toolkit specifically.
...
Here is the single idea this whole book keeps circling back to, so it's worth stating plainly in Chapter 1 rather than letting you discover it piecemeal: **agent judgment is genuinely valuable for open-ended reasoning, and genuinely dangerous for consequential yes/no actions — and Omen's layer boundaries are drawn deliberately along that exact line.**

"Which model family looks like it fits this series' shape, given what Layer 1 found?" is a question worth an agent's judgment. There's no single correct answer, context matters, and a reasoning process that weighs stationarity findings against seasonality findings against residual diagnostics is doing something a fixed rule genuinely can't do as well. Chapters 8 through 13 spend a lot of time building your intuition for exactly this kind of reasoning.
